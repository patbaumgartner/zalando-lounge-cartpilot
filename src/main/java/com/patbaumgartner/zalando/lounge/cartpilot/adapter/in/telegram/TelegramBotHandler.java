 ...
 *
 * Commands (UC-07): /status, /scan, /profiles, /profile …, /help
 *
...
public class TelegramBotHandler {
	...
	public TelegramBotHandler(TelegramClient telegramClient, CartService cartService,
			...
			ProductReservationPort reservationPort, DiscoveredProductPort productPort, ProfilePort profilePort) {
	...
	private void handleMessage(Message message) {
		...
		try {
			if (text.startsWith("/help")) {
				...
				reply(chatId, buildStatusText());
			}
			else if (text.startsWith("/profiles") && isAdmin) {
				reply(chatId, buildProfilesText());
			}
			else if (text.startsWith("/profile") && isAdmin) {
				handleProfileCommand(chatId, text);
			}
			else if (text.startsWith("/scan") && isAdmin) {
				reply(chatId, "🔍 Starting manual scan...");
				new Thread(scannerService::scan).start();
			}
			else if (!isAdmin && (text.startsWith("/scan") || text.startsWith("/profile"))) {
				reply(chatId, "❌ Admin-only command.");
			}
		}
		catch (Exception e) {
	...
	private void handleProfileCommand(String chatId, String text) {
		// /profile show <name>
		...
		if (parts.length < 3) {
			reply(chatId, "Usage: /profile <show|activate|deactivate|set|brand> ...");
			return;
		}

		try {
			switch (parts[1].toLowerCase()) {
				case "show" -> reply(chatId, formatProfile(profileService.show(parts[2])));
				case "activate" -> {
					profileService.activate(parts[2]);
					reply(chatId, "✅ Profile " + parts[2] + " activated.");
				}
				case "deactivate" -> {
					profileService.deactivate(parts[2]);
					reply(chatId, "✅ Profile " + parts[2] + " deactivated.");
				}
				case "set" -> handleProfileSet(chatId, parts);
				case "brand" -> handleProfileBrand(chatId, parts);
				default -> reply(chatId, "Unknown profile subcommand: " + parts[1]);
			}
		}
		catch (ProfileManagementService.ProfileNotFoundException e) {
			reply(chatId, "❌ Profile not found: " + e.getMessage());
		}
	}

	private void handleProfileSet(String chatId, String[] parts) {
		// /profile set size <name> <category> <size>
		// /profile set price <name> <category> <chf>
		if (parts.length < 6) {
			reply(chatId,
		...
		if ("size".equals(subCmd)) {
			...
			reply(chatId, "✅ Size updated: %s %s = %s".formatted(name, category, parts[5]));
		}
		else if ("price".equals(subCmd)) {
			profileService.setMaxPrice(name, category, new BigDecimal(parts[5]));
			reply(chatId, "✅ Max price updated: %s %s = CHF %s".formatted(name, category, parts[5]));
		}
		else {
			reply(chatId, "Unknown set subcommand: " + subCmd);
		}
	}

	private void handleProfileBrand(String chatId, String[] parts) {
		// /profile brand add <name> <tier> <brand>
		// /profile brand remove <name> <brand>
		if (parts.length < 5) {
			reply(chatId, "Usage: /profile brand add <name> <tier1|tier2> <brand> OR remove <name> <brand>");
			return;
		}
		...
		if ("add".equals(action) && parts.length >= 6) {
			...
			reply(chatId, "✅ Brand added: %s → %s (%s)".formatted(brand, name, tier));
		}
		else if ("remove".equals(action)) {
			var brand = String.join(" ", Arrays.copyOfRange(parts, 4, parts.length));
			profileService.removeBrand(name, brand);
			reply(chatId, "✅ Brand removed: %s from %s".formatted(brand, name));
		}
		else {
			reply(chatId, "Usage: /profile brand add|remove ...");
		}
	}
