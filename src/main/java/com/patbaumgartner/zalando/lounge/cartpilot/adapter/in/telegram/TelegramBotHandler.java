...
public class TelegramBotHandler {
	...
	public InjectedCommandResult onInjectedGroupCommand(String text, boolean asAdmin) {
		if (text == null || text.isBlank()) {
			return new InjectedCommandResult(false, "", "text is blank");
		}

		var replies = new StringBuilder();
		handleGroupCommand(text.trim(), asAdmin, "injected-admin", replyText -> {
			if (!replies.isEmpty()) {
				replies.append("\n\n");
			}
			replies.append(replyText);
		});
		return new InjectedCommandResult(true, replies.toString(), null);
	}
	...
	public record InjectedCommandResult(boolean ok, String replies, String error) {
	}
