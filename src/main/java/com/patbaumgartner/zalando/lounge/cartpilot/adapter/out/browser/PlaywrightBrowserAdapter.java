...
public class PlaywrightBrowserAdapter implements BrowserPort {
	...
	public boolean addToCart(String productUrl, String size) {
		try (var page = openPage()) {
			...
			// Give JavaScript time to navigate
			Thread.sleep(500);
			...
			try {
				...
				log.warn("Wait for DOMCONTENTLOADED failed: {}", e.getMessage());
			}
	...
	public int clearCart() {
		try (var page = openPage()) {
			...
			for (int i = 0; i < 100; i++) {
				...
				var removeBtn = page.querySelector(REMOVE_BUTTON_SELECTOR);
				if (removeBtn == null) {
					break;
				}
				...
				try {
					page.click(REMOVE_BUTTON_SELECTOR);
					removed++;
				}
				catch (Exception e) {
					log.warn("Failed to click remove button: {}", e.getMessage());
					// Exit on first error to avoid infinite loops from stale references
					break;
				}
	...
	private String cartUrl() {
		var baseUrl = properties.zalando().baseUrl();
