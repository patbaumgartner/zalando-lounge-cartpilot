...
/**
 * Manages the cart lifecycle (UC-03 + Buy/Skip callback handling).
 *
 * Responsibilities: - Add Tier-1 items to cart via Playwright. - Persist NOTIFY_ONLY
 * reservations without touching the browser. - Handle Buy / Skip Telegram callbacks.
 */
@Service
public class CartService {
	...
	public void addToCart(FilterResult result) {
		...
		if (!added) {
			log.warn("Size {} out of stock for product {}", result.size(), product.name());
