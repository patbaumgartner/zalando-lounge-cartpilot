...
VALUES ('Pat', 'MEN', TRUE, 250.00, 400.00, 200.00,
        'Mammut,Arc''teryx,Patagonia,ECCO,Salomon,Scarpa,Fjällräven,Jack & Jones,Vaude,Dolomite,Peak Performance',
        'Jack Wolfskin,Columbia,Haglöfs,Icebreaker,Merrell,Salewa,Helly Hansen,Smartwool,Bergans of Norway,Berghaus,Schöffel,Odlo,Meindl,Lowa',
        'TNF=The North Face');
...
SELECT id, 'TROUSERS',  '54'    FROM profiles WHERE name = 'Pat' UNION ALL
SELECT id, 'JACKETS',   '54'    FROM profiles WHERE name = 'Pat' UNION ALL
SELECT id, 'UNDERWEAR', 'L'     FROM profiles WHERE name = 'Pat' UNION ALL
SELECT id, 'SWIMWEAR',  'L'     FROM profiles WHERE name = 'Pat' UNION ALL
SELECT id, 'BELTS',     '105'   FROM profiles WHERE name = 'Pat' UNION ALL
