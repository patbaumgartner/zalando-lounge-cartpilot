...
│       │                                             │
│       ├──► CartKeepAliveService (15-min refresh)     │
│       │                                             │
│       ▼                                             │
...
1. Playwright removes the item and immediately re-adds it. Mutating the basket resets Zalando's server-side reservation timer (a read-only check does not), genuinely prolonging the hold.
2. If it can no longer be re-added (sold out or bought outside the app) → mark `EXPIRED`; post to group: `⚠️ [Item] for [Profile] expired — no action taken.`
