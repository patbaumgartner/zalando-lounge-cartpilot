...
- Maven
- Microsoft Playwright (Java) – headless Chromium, single browser instance, multiple tabs
- rubenlagus/TelegramBots 7.x
- Spring Scheduler (`@Scheduled`)
- Spring Data JDBC + Flyway — PostgreSQL (prod) / H2 (dev / tests)
- Docker (single container, env-var driven)
...
1. On first run: Playwright logs in with credentials from env vars; saves browser storage state to `session.json`.
2. Every subsequent run: loads `session.json` (cookies + local storage) before navigating.
3. Before each scan: validates session by navigating to a protected page; re-logs in and overwrites `session.json` if invalid.
...
BRAND_ALIASES=TNF=The North Face,...   # system-wide aliases
